...
unlink('Causal_Impact2_cache', recursive = TRUE)
...
## Time-series models & Backtesting approach
...
For the meantime, we selected the model with the lowest cumulative errors, which assumes a Studentized distribution and a standard deviation of .01 in the priors. We compared it with the same model but applied to raw data, in order to back-transform the log counts into more meaningful results.

<br>

```{r bsts_model_selected, echo=T, cache= T, paged.print=TRUE, warning=F, eval=T, fig.show="hide"}
#plot(model2d1,"prediction.errors")
...
ss2d_nolog <- list()
# Local trend, weekly-seasonal #https://qastack.mx/stats/209426/predictions-from-bsts-model-in-r-are-failing-completely - PUSE UN GENERALIZED LOCAL TREND
ss2d_nolog <- AddLocalLevel(ss2d_nolog, data15a64_rn_causal$hosp_trauma) #
# Add weekly seasonal
ss2d_nolog <- AddSeasonal(ss2d_nolog, data15a64_rn_causal$hosp_trauma,nseasons=5, season.duration = 52) #weeks OJO, ESTOS NO SON WEEKS VERDADEROS. PORQUE TENGO MAS DE EUN AÑO
ss2d_nolog <- AddSeasonal(ss2d_nolog, data15a64_rn_causal$hosp_trauma, nseasons = 12, season.duration =4) #years
#ss2 <- AddAutoAr(ss2, y = data15a64_rn_causal$log_hosp_trauma, lags = 1) #NO PUEDO AREGAR AR1 CON POISSON
# For example, to add a day-of-week component to data with daily granularity, use model.args = list(nseasons = 7, season.duration = 1). To add a day-of-week component to data with hourly granularity, set model.args = list(nseasons = 7, season.duration = 24).
model2d1_nolog <- bsts(data15a64_rn_causal$hosp_trauma ~ data15a64_rn_causal$hosp_circ+  data15a64_rn_causal$hosp_resp+ data15a64_rn_causal$difftrc,
               state.specification = ss2d_nolog, #A list with elements created by AddLocalLinearTrend, AddSeasonal, and similar functions for adding components of state. See the help page for state.specification.
               family ="student", #A Bayesian Analysis of Time-Series Event Count Data
               niter = clus_iter, 
              # burn = 500, #http://finzi.psych.upenn.edu/library/bsts/html/SuggestBurn.html Suggest the size of an MCMC burn in sample as a proportion of the total run.
               seed= 2125)
#,
#               dynamic.regression=T)
plot(model2d1_nolog, main = "Linear Trend of the Selected Model, But without Log-Transformation")
plot(model2d1_nolog, "components")
mtext(side=3, line=2, at=-0.07, adj=0, cex=1.7, "Components of the Selected Model, But without Log-Transformation")

post_period_response_nolog <- data15a64_rn$hosp_trauma[253:262]

impact3d1_nolog <- CausalImpact(bsts.model = model2d1_nolog,
                       post.period.response = post_period_response_nolog)

burn2d1_nolog <- SuggestBurn(0.1, model2d1_nolog)
```
...
impact3d1_final<-impact3d1
summary(impact3d1_final,"report")
impact3d1_final<-impact3d1
write.csv2(impact3d1_final$series, "data.csv")
impact3d1_final$series<-exp(impact3d1_final$series)

impact3d1_final$summary$Actual<-exp(impact3d1_final$summary$Actual) #4.389589 43.895893
impact3d1_final$summary$Pred<-exp(impact3d1_final$summary$Pred)
impact3d1_final$summary$Pred.lower<-exp(impact3d1_final$summary$Pred.lower)
impact3d1_final$summary$Pred.upper<-exp(impact3d1_final$summary$Pred.upper)
impact3d1_final$summary$Pred.sd<-exp(impact3d1_final$summary$Pred.sd)
impact3d1_final$summary$AbsEffect<-exp(impact3d1_final$summary$AbsEffect)
impact3d1_final$summary$AbsEffect.lower<-exp(impact3d1_final$summary$AbsEffect.lower)
impact3d1_final$summary$AbsEffect.upper<-exp(impact3d1_final$summary$AbsEffect.upper)
impact3d1_final$summary$AbsEffect.sd<-exp(impact3d1_final$summary$AbsEffect.sd)
impact3d1_final$summary$RelEffect<-exp(impact3d1_final$summary$RelEffect)
impact3d1_final$summary$RelEffect.lower<-exp(impact3d1_final$summary$RelEffect.lower)
impact3d1_final$summary$RelEffect.upper<-exp(impact3d1_final$summary$RelEffect.upper)
impact3d1_final$summary$RelEffect.sd<-exp(impact3d1_final$summary$RelEffect.sd)

plot(impact3d1)

plot(impact3d1_nolog)
#one_step_errors <- colMeans(impact3d1$one.step.prediction.errors)
#plot(one_step_errors, type="l")

summary(impact3d1_nolog)
print("")
print("Log transformed Report")
summary(impact3d1_final)
...
We estimated the difference in post treatment and pretreatment periods while controlling for covariates and a lagged depentent variable through a Type-2 Sum Squares ANCOVA Lagged Dependent Variable model. The model also accounts for baseline levels and trends present in the data, allowing us to attribute significant changes to the interruption. The F-statistics were calculated using a bootstrap model of `r print(clus_iter)` replications.
...
itsa_metro_region_quar<-
        its.analysis::itsa.model(time = "rn", depvar = "log_hosp_trauma",data=data15a64_rn2,
                                 interrupt_var = "did", covariates = c("log_hosp_circ", "log_hosp_resp","difftrc"), 
                                 alpha = 0.05,no.plots = FALSE, bootstrap = TRUE, Reps = clus_iter, print = TRUE) 
...
There were significant differences in the average trauma hospitalizations (F=`r round(itsa_metro_region_quar$booted.ints$'Mean F-value',2)[1]`, p=`r round(itsa_metro_region_quar$booted.ints$'P-value',3)[1]`). The differences explained `r scales::percent(itsa_metro_region_quar$adjr_sq)` of the variances. Must note that there were no autocorrelation evidence, but there were abnormal residuals (p= `r itsa_metro_region_quar$shapiro.test`). Tukey HSD test showed significant differences `r paste0("(diff=",round(exp(itsa_metro_region_quar$tukey.result$'x$interrupt_var'[1]),2),",p=",sprintf("%4.4f",itsa_metro_region_quar$tukey.result$'x$interrupt_var'[4]),")")` between pre-period (`r paste0("M=",sprintf("%4.2f",itsa_metro_region_quar$group.means[1,3]),", SD=",sprintf("%4.2f",itsa_metro_region_quar$group.means[1,4]))`) and post-intervention period (`r paste0("M=",sprintf("%4.2f",itsa_metro_region_quar$group.means[2,3]),", SD=",sprintf("%4.2f",itsa_metro_region_quar$group.means[2,4]))`).

<br>

# Synthetic Control

Next, we compared the log-transformed data of trauma hospitalizations in 2019, to the log-transformed hospitalizations of years from 2015 to 2018, estimating a counterfactual based on the weeks previous to the event (from week 1 to 42).

<br>

```{r exp_plot_lines_log_synth, echo=T, cache= T, paged.print=TRUE, warning=F, fig.height=14,eval=T, fig.align='center', fig.cap= "Figure 13. Linear trends of Log-transformed Weekly Hospital Traumatizations by Years (Median and IQRs)"}
data15a64_rn2%>%
 # dplyr::group_by(year,yearweek)%>%
  #dplyr::summarise(median=median(hosp_trauma,na.rm=T))%>%
ggplot() + #median
  ...
  theme(strip.text.x = element_text(size = 14, face = "bold"),
        ...
        legend.position = "none",
        plot.caption=element_text(hjust = 0))+
  geom_smooth(aes(y = log_hosp_trauma,x = isoweek, color = year),stat = 'summary', color = 'red', fill = 'red', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  geom_smooth(aes(y = log_hosp_circ,x = isoweek, color = year),stat = 'summary', color = 'darkblue', fill = 'navyblue', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  geom_smooth(aes(y = log_hosp_resp,x = isoweek, color = year),stat = 'summary', color = 'violet', fill = 'violet', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  geom_smooth(aes(y = log_difftrc,x = isoweek, color = year),stat = 'summary', color = 'darkgreen', fill = 'green', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
    #  geom_smooth(aes(y = hosp_total,x = yearweek, color = year),stat = 'summary', color = 'gray80', fill = 'gray80', alpha = 0.2, 
   #             fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  labs(caption="Note. Year 2015 & 2016 had a length of 53 weeks.\nRed= Trauma Hospitalizations (log); Blue= Circulatory System Hospitalizations (log); Violet= Respiratory System Hospitalizations (log); Green= Diff. Between Trauma and Total Consultations (log)")+
  xlab("Week")+
  scale_x_continuous(
  breaks = seq(from = 1, to = 53, by =4)#,
#  label = c("two", "four", "six")
...
```{r exp_plot_lines_log_synth_std_diff, echo=T, cache= T, paged.print=TRUE, warning=F, fig.height=14,eval=T, fig.align='center', fig.cap= "Figure 14. Linear trends of Std. Differences of Log-Transformed Variables, Between 2015-2018 vs. 2019, Before Week 43"}
ggplotly_trends<-data15a64_rn2%>%
   dplyr::filter(isoweek<43) %>%
  #escalar los datos por todos l
  dplyr::group_by(isoweek) %>% 
  dplyr::mutate_at(.vars=vars(log_hosp_trauma, log_hosp_circ, log_hosp_resp, log_difftrc), 
                   .funs = funs(`zw`=scale(.)))%>%
  dplyr::select(year,log_hosp_trauma, log_hosp_circ, log_hosp_resp, log_difftrc) %>% 
  dplyr::ungroup()%>%
  dplyr::mutate(treat.var=ifelse(year==2019,1,0))%>%
  dplyr::group_by(treat.var,isoweek)%>%
  dplyr::summarise_at(.vars=vars(log_hosp_trauma, log_hosp_circ, log_hosp_resp, log_difftrc),
...
#  dplyr::mutate(sig=factor(sig))%>%
  ...
  geom_line(size=1)+
  geom_point(size=2) +
  labs(y = "Std. Differences (Log-transformed)", x = "Week Number")+ 
...
dataprep.out <-
        dataprep(data15a64_rn3,
          predictors = c("log_hosp_circ", "log_hosp_resp"),
          dependent     = "log_hosp_trauma",
          unit.variable = "year",
          time.variable = "isoweek",
          treatment.identifier  = 2019,
          special.predictors = list(
             list("log_hosp_trauma", 1, "median"),
             list("log_hosp_trauma", 11, "median"),
             list("log_hosp_trauma", 21, "median"),
             list("log_hosp_trauma", 41, "median")),
...
plot.df = rbind(data.frame(year=years,y=plot.df$w.weight,unit='Synthetic'),
           data.frame(year=years,y=data15a64_rn3$log_hosp_trauma[data15a64_rn3$year==2019 & data15a64_rn3$isoweek %in% years],unit='2019')
)
...
- I still do not know much about the control variables chosen, and if there is any justification for that selection (eg. diff trauma consultations- total consultations).
...
- Brodersen recommends using at least 5 control variables [(in a conference in this link)](https://www.youtube.com/watch?v=GTgZfCltMm8&feature=emb_imp_woyt). But as could be seen on the synthetic control method, two of them seems to differ to the hospitalizations.
- Poisson distributional assumptions based on counts (non-negative and integer predictions)
