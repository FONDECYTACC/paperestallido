...
# Exploration of Time Series of Respiratory Consultations
...
tsData1 <- ts(data15a64_rn$cons_resp, frequency=1) #a cada unidad temporal le asigno una observación, por lo que asume que la unidad temporal considerada es el día
tsData4 <- ts(data15a64_rn$cons_resp, frequency=4) #a cada unidad temporal le asigno una observación, por lo que asume que la unidad temporal considerada es el día
tsData52.4 <- ts(data15a64_rn$cons_resp, frequency=52.4)#a cada unidad temporal le asigno una observación, por lo que asume que la unidad temporal considerada es la semana (262) #53+53+52+52+52
tsData17.5 <- ts(data15a64_rn$cons_resp, frequency=17.5)#a cada unidad temporal le asigno una observación, por lo que asume que la unidad temporal considerada es la semana (262) #53+53+52+52+52
tsData_m <-msts(data15a64_rn$cons_resp, seasonal.periods=c(4,52.5)) #a cada unidad temporal le asigno una observación, por lo que asume que la unidad temporal considerada es el día
...
ggAcf(data15a64_rn$cons_resp,20,main="3.a.Autocorreation Plot of the Series of Respiratory Consultations")+ #data15a64_rn$hosp_trauma
  theme_sjplot()+
...
ggPacf(data15a64_rn$cons_resp,20,main="3.b.Partial Autocorreation Plot of the Series of Respiratory Consultations")+ #data15a64_rn$hosp_trauma
  theme_sjplot()+
  theme(plot.caption=element_text(hjust = 0))+
  labs(x="Number of Lags",y="ACF",caption="Note. Dotted Blue Line= 95% CI")
  #ylim(-.25,.25)
...
As seen in the correlograms and in the multiple decompositions, the series contains an upward trend, leading to think that the mean was not constant over time, and there could be evidence of seasonality, and correlation every ~25 days. This could be interpreted as that the series were not stationary. Additionally, partial autocorrelation may let us suspect of an autoregressive term of order 2.  However, the Dickey-Fuller test indicated the presence of stationarity (`r round(tseries::adf.test(tsData1)$statistic,2)`, p= `r round(tseries::adf.test(tsData1)$p.value,3)`).
...
y_pois<-data15a64_rn_causal$cons_resp[1:(length(data15a64_rn_causal$cons_resp)-10)]
...
x3_pois<-data15a64_rn_causal$diffreh[1:(length(data15a64_rn_causal$diffreh)-10)]
...
x_pois<-data15a64_rn[length(data15a64_rn$diffreh)-10:length(data15a64_rn$diffreh),c("cons_resp","cons_circ","cons_resp","diffreh")]
...
itsa_metro_region_quar<-
        its.analysis::itsa.model(time = "rn", depvar = "cons_resp",data=data15a64_rn2,
                                 interrupt_var = "did", covariates = c("cons_circ", "cons_trauma","diffreh"), 
...
```{r itsa_plot,echo=T,fig.align='center', fig.pos='H', eval=T, message=FALSE, warning=F, fig.align='center', fig.cap= "Figure 12. Interrupted Time-Series Analysis of Respiratory Consultations in Inervention Period (2019-10-18)", error=T}
...
There were non-significant differences in the average respiratory consultations (F=`r round(itsa_metro_region_quar$booted.ints$'Mean F-value',2)[1]`, p=`r round(itsa_metro_region_quar$booted.ints$'P-value',3)[1]`). Must note that there were no autocorrelation evidence, there were normal residuals (p= `r itsa_metro_region_quar$shapiro.test`).Variances were not equal. Tukey HSD test showed non-significant differences `r paste0("(diff=",round(itsa_metro_region_quar$tukey.result$'x$interrupt_var'[1],2),",p=",sprintf("%4.4f",itsa_metro_region_quar$tukey.result$'x$interrupt_var'[4]),")")` between pre-period (`r paste0("M=",sprintf("%4.2f",itsa_metro_region_quar$group.means[1,3]),", SD=",sprintf("%4.2f",itsa_metro_region_quar$group.means[1,4]))`) and post-intervention period (`r paste0("M=",sprintf("%4.2f",itsa_metro_region_quar$group.means[2,3]),", SD=",sprintf("%4.2f",itsa_metro_region_quar$group.means[2,4]))`).

<br>

# Synthetic Control

Next, we compared respiratory consultations in 2019, to the consultations of years from 2015 to 2018, estimating a counterfactual based on linear trends from these years and using the weeks previous to the event (from week 1 to 42).

<br>

```{r exp_plot_lines_synth, echo=T, cache= T, paged.print=TRUE, warning=F, fig.height=14,eval=T, fig.align='center', fig.cap= "Figure 13. Linear trends of Weekly Respiratory Consultations by Years (Median and IQRs)"}
...
ggplot() + #median
  ...
  geom_smooth(aes(y = cons_resp,x = isoweek, color = year),stat = 'summary', color = 'red', fill = 'red', alpha = 0.2, 
  ...
  geom_smooth(aes(y = cons_circ,x = isoweek, color = year),stat = 'summary', color = 'darkblue', fill = 'navyblue', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  geom_smooth(aes(y = cons_trauma,x = isoweek, color = year),stat = 'summary', color = 'violet', fill = 'violet', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  geom_smooth(aes(y = diffreh,x = isoweek, color = year),stat = 'summary', color = 'darkgreen', fill = 'green', alpha = 0.2, 
                fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
    #  geom_smooth(aes(y = hosp_total,x = yearweek, color = year),stat = 'summary', color = 'gray80', fill = 'gray80', alpha = 0.2, 
   #             fun.data = median_hilow, fun.args = list(conf.int = 0.5))+
  labs(caption="Note. Year 2015 & 2016 had a length of 53 weeks.Red= Trauma Consultations; Blue= Circulatory System Consultations ;\nViolet= Respiratory System Consultations; Green= Diff. Between Respiratory and Total Hospitalizations")+
  xlab("Week")+
  scale_x_continuous(
...
ggplotly(ggplotly_series)%>% 
 layout(annotations = 
 list(x = .01, y = -0.038, text = "Note. Year 2015 & 2016 had a length of 53 weeks.Red= Trauma Consultations; Blue= Circulatory System Consultations;\nViolet= Respiratory System Consultations; Green= Diff. Between Respiratory and Total Hospitalizations", 
...
```{r exp_plot_lines_synth_std_diff, echo=T, cache= T, paged.print=TRUE, warning=F, fig.height=14,eval=T, fig.align='center', fig.cap= "Figure 14. Linear trends of Std. Differences of Variables, Between 2015-2018 vs. 2019, Before Week 43"}
ggplotly_trends<-data15a64_rn2%>%
  ...
  dplyr::mutate_at(.vars=vars(cons_resp, cons_circ, cons_trauma, diffreh), 
                   .funs = funs(`zw`=scale(.)))%>%
  dplyr::select(year,cons_resp_zw, cons_circ_zw, cons_trauma_zw, diffreh_zw) %>% 
  dplyr::ungroup()%>%
  dplyr::mutate(treat.var=ifelse(year==2019,1,0))%>%
  dplyr::group_by(treat.var,isoweek)%>%
  dplyr::summarise_at(.vars=vars(cons_resp_zw, cons_circ_zw, cons_trauma_zw, diffreh_zw), 
...
#cons_trauma, cons_circ, cons_resp, difftrh
dataprep.out <-
        dataprep(data15a64_rn3,
          predictors = c("cons_circ", "cons_trauma","diffreh"),
          dependent     = "cons_resp",
          unit.variable = "year",
          time.variable = "isoweek",
          treatment.identifier  = 2019,
          special.predictors = list(
             list("cons_resp", 1, "median"),
             list("cons_resp", 11, "median"),
             list("cons_resp", 21, "median"),
             list("cons_resp", 41, "median")),
...
gaps.plot(synth.res = synth2.out, dataprep.res = dataprep.out, Ylab = "Respiratory Consultations", Xlab= "Week Number")
```

<br>

```{r synth3, echo=T, cache= T, paged.print=TRUE, warning=F, eval=T, fig.align='center', fig.cap= "Figure 16. Comparison of Linear Trends of Respiratory Consultations in 2019 vs. Synthetic Counterfactual Based on 2015-2018"}
...
plot.df = rbind(data.frame(year=years,y=plot.df$w.weight,unit='Synthetic'),
           data.frame(year=years,y=data15a64_rn3$cons_resp[data15a64_rn3$year==2019 & data15a64_rn3$isoweek %in% years],unit='2019')
)
ggplot(plot.df,aes(x=year,y=y,color=unit)) + geom_line() + ylab("") + xlab("Week Number") + scale_color_manual(values=c('black','red')) +
  ggtitle('Respiratory Consultations') +
  theme_sjplot2()+
...
# omit the following lines, if your missing values are already properly labelled
      ...
      tryCatch(
                     {
      save.image(paste0(getwd(),"/","Procesos hasta 5_cons_resp.RData"))
