...
A total of `r format(as.numeric(sum_cons_hosp[1]),big.mark=",")` ED consultations and `r format(as.numeric(sum_cons_hosp[2]),big.mark=",")` hospitalizations  were observed from October 21 of 2019 to December 31, 2019. Overall, during 2015-2019 trauma cases represent the `r as.character(ratio_cons_hosp["cons_trauma_perc"])` of total consultation and `r as.character(ratio_cons_hosp["hosp_trauma_perc"])` of total hospitalizations, and respiratory represents the `r as.character(ratio_cons_hosp["cons_resp_perc"])` and `r as.character(ratio_cons_hosp["hosp_resp_perc"])`, respectively . 
...
Data is available in the following  [link]("https://github.com/FONDECYTACC/paperestallido/blob/main/data15a64_rn_ratio_its.dta?raw=true") (https://github.com/FONDECYTACC/paperestallido/blob/main/data15a64_rn_ratio_its.dta?raw=true).
...
data_did<-
  cbind.data.frame(
y=c(rep("hosp_trauma",5), rep("hosp_resp",5), rep("cons_trauma",5),rep("cons_resp",5),rep("rate",5),rep("rate_resp",5)),
x=rep(c("tx*txtime","tx*txtime + month","tx*txtime + factor(month)","tx*txtime + month*month","tx*txtime + month_cos + month_sin"),6),
mod_name=c("HT_none","HT_cont","HT_fact","HT_cuad","HT_cos_sin",
           "HR_none","HR_cont","HR_fact","HR_cuad","HR_cos_sin",
           "CT_none","CT_cont","CT_fact","CT_cuad","CT_cos_sin",
           "CR_none","CR_cont","CR_fact","CR_cuad","CR_cos_sin",
           "RT_none","RT_cont","RT_fact","RT_cuad","RT_cos_sin",
           "RR_none","RR_cont","RR_fact","RR_cuad","RR_cos_sin"))


for (i in  1:30) {
  assign(paste0(data_did[i,"mod_name"],"_DiD"), plm::plm(as.formula(paste0(data_did[i,1],"~ ",data_did[i,2])), data=data15a64_rn_ratio_its, index=c("year", "isoweek"), model="within"),#effect= "individual"
         envir = .GlobalEnv)

  assign(paste0("wi_",data_did[i,"mod_name"],"_DiD"),
         plm::within_intercept(get(paste0(data_did[i,"mod_name"],"_DiD")),return.model=T))
}


  assign(paste0("vcov_",data_did[i,"mod_name"],"_DiD"),lmtest::coeftest(get(paste0("wi_",data_did[i,1],"_DiD")),
                                                               vcov. = function(x) vcovSCC(x, type="sss")))
#https://stats.stackexchange.com/questions/66973/difference-between-fixed-effects-models-in-r-plm-and-stata-xtreg
  
  wi<-within_intercept(RR_cos_sin_DiD, return.model=T)
  
  lmtest::coeftest(femodelint, vcov. = function(x) vcovSCC(x, type = "sss"))

#stargazer(type = "html",HT_fact_DiD,  single.row = TRUE, header = FALSE, digits=5)
#$HT_fact_DiD
```

```{r 10_did2, messages=T, eval=F, warnings=T}
...
AICs_BICs_fes<-data.frame()
  for (i in  1:30) {
    AICs_BICs_fes<-rbind(AICs_BICs_fes,c(aicbic_plm(get(paste0(data_did[i,"mod_name"],"_DiD")),"AIC"),
      aicbic_plm(get(paste0(data_did[i,"mod_name"],"_DiD")),"BIC")))
  }
colnames(AICs_BICs_fes)<-c("AIC","BIC")


lmtest::coeftest(get("wi_",paste0(data_did[i,"mod_name"],"_DiD")),vcov. = function(x) vcovSSC(x,type="sss"))
