...
comp_mod_df_sum<-
comp_mod_df %>% dplyr::group_by(Models) %>% summarise(mean=mean(abs(`Cumulative Absolute Error`),na.rm=T),median=median(abs(`Cumulative Absolute Error`),na.rm=T), p25=quantile(abs(`Cumulative Absolute Error`),.25,na.rm=T),p75=quantile(abs(`Cumulative Absolute Error`),.75,na.rm=T), max=max(`Cumulative Absolute Error`,na.rm=T)) %>%
