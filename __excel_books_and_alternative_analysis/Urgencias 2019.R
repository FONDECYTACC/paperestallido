...
remove(list=ls())
library(readr)
establecimientos <- read_delim("Documents/Universidad/Otros trabajos propios y de hospital/Bases de datos poblacionales/Atenciones de Urgencia Chile/establecimientos.csv", ";", escape_double = FALSE, trim_ws = TRUE)

View(establecimientos)
library(Hmisc)
urg_2019 <- mdb.get("AtencionesUrgencia2019.mdb")
View(urg_2019)
...
consultas_total<- urg_2019[urg_2019$Idcausa==1 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
consultas_respiratorio<- urg_2019[urg_2019$Idcausa==2 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
consultas_circulatorio<- urg_2019[urg_2019$Idcausa==12 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
consultas_diarrea<- urg_2019[urg_2019$Idcausa==29 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
consultas_trauma<- urg_2019[urg_2019$Idcausa==18 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
hosp_total<- urg_2019[urg_2019$Idcausa==25 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
hosp_respiratorio<- urg_2019[urg_2019$Idcausa==7 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
hosp_circulatorio<- urg_2019[urg_2019$Idcausa==22 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
hosp_trauma<- urg_2019[urg_2019$Idcausa==23 & urg_2019$idestablecimiento %in% estab_RM, columnas, drop=FALSE]
...
BD_urg_2019_RM <- merge(consultas, hospitalizaciones)
library(foreign)
write.dta(BD_urg_2019_RM, "/Users/masterin/Documents/Universidad/Otros trabajos propios y de hospital/Bases de datos poblacionales/Atenciones de Urgencia Chile/BD_urg_2019_RM.dta")

# ----------- acotando hospitales x mediana ---------
library(dplyr)
medianas <- summarise(group_by(BD_urg_2019_RM, id), median = median(hosp_trauma_15a64))
entra <- medianas[medianas$median>0, "id"]
nombres <- establecimientos[establecimientos$codigo_antiguo %in% entra$id, "nombre"]
BD_urg_2019_RM_trauma <-BD_urg_2019_RM[BD_urg_2019_RM$id %in% entra$id, ]
BD_urg_2019_RM_trauma$fecha<- as.character.Date(BD_urg_2019_RM_trauma$fecha)
library(foreign)
write.dta(BD_urg_2019_RM_trauma, "/Users/masterin/Documents/Universidad/Otros trabajos propios y de hospital/Bases de datos poblacionales/Atenciones de Urgencia Chile/BD_urg_2019_RM-trauma.dta")
